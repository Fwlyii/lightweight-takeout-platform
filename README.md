...
- 最新版本唯一正式入口：<https://elm-demo.pages.dev/index>。正式发布使用 Cloudflare Pages 项目 `elm-demo` 的 `main` 分支；历史对比环境和临时部署地址都不是正式入口。
- 页面与固定导航共用 `--app-width`（桌面 600px，手机自适应），UI 以 `fwl-AI` 为准，保留主线业务校验。普通详情使用 `PageHeader`，避免重复返回按钮；页面保留底部操作栏和安全区的滚动空间。
- 商家点餐和 AI 点餐共用购物车、确认订单、支付流程；多商家购物车需先选择一家商家，禁止合并为跨商家订单。订单金额仍由后端核算。
- 支付页按订单、优惠、支付方式分区，钱包余额与积分均由后端校验；支付宝和微信尚未接入真实支付渠道。支付期间禁止重复提交和更换优惠券。
- 路由实例按完整地址区分，切换订单或商家参数会重新加载对应数据，不复用上一笔订单的支付状态。

### 合并 AI 前的独立对比环境

对比地址：<https://elm-pre-ai.whliugong.xyz/>。历史提交为 `547bb6b2a8f4a3e17e95f6bfc89930d620249037`（合并 `fwl-AI` 的第一父提交），前后端保持该提交内容，公网同源代理单独处理历史 CORS 限制。

该环境运行在本机 Docker，前端端口 `18091`、后端端口 `18090`，使用独立的 `elm-preai-comparison` Compose 项目、数据库和上传卷，不使用 Railway，也不共享当前环境业务数据。本机或 Docker 停止时对比站点不可用；Quick Tunnel 重建后需将对比 Worker 的 `COMPARISON_ORIGIN` 更新为新地址。
...
主题一致性修复：暖阳橙／薄荷青使用 `--skin-*` 语义色与 RGB 伴随值，覆盖搜索、标签、购物栏、AI、个人中心及商家／骑手／管理端的界面配色。默认蓝白回退色保持不变，主题预览色块、商品图片及成功／警告／错误提示保留本来颜色；不再通过通配 `header` 强制染色。首次挂载前恢复本地主题。新增 `skin-regressions.test.mjs` 防止重新引入硬编码蓝色。

路由内容不再直接包裹 `out-in Transition`：订单页包含多个根节点，会令离场等待卡住并造成后续页面空白。验收需要连续点击「首页→订单→我的」、切换主题并测试浏览器前进／后退，不能只刷新直达各页。
