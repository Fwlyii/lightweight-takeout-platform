<template>
  <ProfilePage title="测试商家">
    ...
    <article v-for="store in stores" :key="store.id" class="panel">
      ...
      <p>
        {{ store.score == null ? "暂无评分" : store.score + "分" }} · 月售{{
          store.salesCount
        }}单
      </p>
      <button type="button" :disabled="busy" @click="collect(store.id)">
        收藏{{ store.businessName }}
      </button>
    </article>
...
onMounted(async () => {
  try {
    stores.value = await searchBusinesses("");
  } catch (e) {
...
async function collect(id) {
  busy.value = true;
  try {
    await updateMyInteraction({ merchantId: id, collected: true });
    message.value = "已收藏";
  } catch (e) {
    error.value = e.message;
  } finally {
