<template>
    <div class="ai-chat-page">
        ...
        <section class="smart-order-bar">
            ...
            <div class="smart-order-form"><input v-model="smartQuery" placeholder="例如：清淡、牛肉、早餐"
                    @keyup.enter="findSmartFoods"><input v-model="smartBudget" type="number" min="1"
                    placeholder="预算¥"><button @click="findSmartFoods" :disabled="smartLoading">{{ smartLoading ? '匹配中' :
                        '开始匹配' }}</button></div>
            <div v-if="smartFoods.length" class="smart-results">
                <div v-for="food in smartFoods" :key="food.foodId" class="smart-food">
                    <div><b>{{ food.foodName }}</b><span>{{ food.businessName }} · ¥{{ food.price }}</span></div><button
                        @click="addSmartToCart(food)">加入购物车</button>
                </div>
            </div>
        </section>
                        ...
                        <div class="quick-questions">
                            ...
                            <div class="question-grid">
                                <button v-for="question in quickQuestions" :key="question"
                                    @click="askQuickQuestion(question)" class="question-btn">
                                    {{ question }}
                                </button>
                            </div>
                        </div>
            ...
            <div class="chat-input">
                <div class="input-container">
                    <div class="input-wrapper">
                        <textarea v-model="inputMessage" @keydown="handleKeyDown" @input="onInputChange"
                            placeholder="输入您的问题，按Enter发送..." :disabled="isTyping" ref="messageInput" rows="1"
...
.smart-order-form input[type=number] {
    max-width: 78px
}

.smart-order-form button,
.smart-food button {
    border: 0;
    border-radius: 6px;
    background: #168bd1;
...
@media(max-width:375px) {
    ...
    .smart-order-form input:first-child {
        flex-basis: 100%
    }
