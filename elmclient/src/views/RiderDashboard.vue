          ...
          <article v-for="task in visibleTasks" :key="task.id" class="task-card" :class="task.taskStatus.toLowerCase()">
            ...
            <div class="task-actions">
              ...
              <template v-else-if="task.taskStatus === 'ACCEPTED'">
                <button class="ghost" @click="navigate(task)"><i class="fas fa-location-arrow"></i> 导航去商家</button><button class="accept" @click="act(task, 'arrive-store')">我已到店</button>
              </template>
              ...
              <template v-else-if="task.taskStatus === 'DELIVERING'">
                <button class="ghost" @click="navigate(task)"><i class="fas fa-location-arrow"></i> 导航去顾客</button><button class="accept" @click="act(task, 'deliver')">确认送达</button>
...
const navigate = async task => {
  const popup = window.open("about:blank", "_blank");
  try {
    const response = await request.get(`/api/v1/delivery-tasks/${task.id}/navigation`);
    const navigation = response.data;
    if (!response.success || !navigation?.navigationUrl) throw new Error(response.message || "导航地址不可用");
    if (popup) {
      popup.opener = null;
      popup.location.replace(navigation.navigationUrl);
    } else {
      toast.info(`请在地图中搜索：${navigation.address}`);
    }
  } catch (error) {
    popup?.close();
    const fallbackAddress = task.taskStatus === "DELIVERING" ? task.deliveryAddress : task.businessAddress;
    toast.error(`${error.response?.data?.message || error.message || "导航服务不可用"}。地址：${fallbackAddress || "未提供"}`);
  }
};
