...
<style scoped>
.manage-user-container { width: 100%; max-width: 960px; margin: 0 auto; min-height: 100%; background: var(--skin-surface, #f5f8fb); color: var(--skin-ink, #253f54); padding-bottom: calc(84px + env(safe-area-inset-bottom)); }
.container { width: 100%; }
.search-section, .filter-section, .user-list { margin: 16px; }
.search-box { display: flex; gap: 10px; align-items: center; padding: 12px 14px; background: #fff; border: 1px solid var(--skin-border, #dbe5ed); border-radius: 10px; }
.search-box input { width: 100%; min-width: 0; border: 0; background: transparent; color: inherit; font: inherit; outline-offset: 4px; }
.filter-tabs { display: flex; gap: 4px; background: var(--skin-border, #eaf0f5); padding: 4px; border-radius: 10px; }
.filter-tab { flex: 1; min-height: 44px; border: 0; padding: 10px 4px; border-radius: 7px; text-align: center; color: var(--skin-muted, #526d82); background: transparent; font: inherit; cursor: pointer; }
.filter-tab.active { background: #fff; color: var(--skin-brand, #087ecc); box-shadow: 0 2px 5px rgba(var(--skin-brand-strong-rgb, 32, 76, 104), 0.070588); }
.user-list { display: grid; gap: 12px; }
.user-item { display: flex; align-items: center; gap: 14px; padding: 16px; background: #fff; border: 1px solid var(--skin-border, #e0e9f0); border-radius: 12px; }
.user-avatar { flex: 0 0 44px; width: 44px; height: 44px; display: grid; place-items: center; background: var(--skin-surface, #e9f5ff); border-radius: 50%; color: var(--skin-brand, #087ecc); }
.user-info { min-width: 0; flex: 1; overflow-wrap: anywhere; }
.user-name { font-weight: 650; font-size: 16px; margin-bottom: 6px; }
.user-details, .user-status { display: flex; flex-wrap: wrap; gap: 6px 12px; color: var(--skin-muted, #647c8d); font-size: 13px; line-height: 1.6; }
...
.action-btn, .modal-btn { min-height: 40px; padding: 8px 14px; border-radius: 8px; border: 1px solid var(--skin-border, #cbdde9); background: #fff; color: var(--skin-brand, #087ecc); font: inherit; cursor: pointer; }
.disable-btn { color: #b54050; border-color: #f0cbd1; }
button:disabled { opacity: .5; cursor: wait; }
.empty-state, [role="status"] { padding: 22px 16px; text-align: center; color: var(--skin-muted, #718798); }
.modal-overlay { position: fixed; inset: 0; z-index: 2000; display: grid; place-items: center; padding: 16px; background: rgba(var(--skin-brand-strong-rgb, 16, 40, 57), 0.501961); }
...
.close-btn { font-size: 24px; border: 0; color: var(--skin-muted, #597285); background: transparent; cursor: pointer; min-width: 40px; min-height: 40px; }
.modal-body { padding: 18px 0; overflow-wrap: anywhere; line-height: 1.6; }
.modal-footer { justify-content: flex-end; }
.confirm-btn { color: white; background: var(--skin-brand, #087ecc); }
