...
public interface AiChatHistoryMapper {
     ...
     * 根据会话ID查询对话记录
     */
    @Select("SELECT * FROM ai_chat_history WHERE session_id = #{sessionId} AND user_id = #{userId} AND is_deleted = 0 " +
            "ORDER BY create_time ASC, id ASC")
    List<AiChatHistory> selectBySessionIdAndUserId(@Param("sessionId") String sessionId,
                                                   @Param("userId") Long userId);

    @Select("SELECT * FROM ai_chat_history WHERE session_id = #{sessionId} AND is_deleted = 0 " +
            "ORDER BY create_time ASC, id ASC")
    List<AiChatHistory> selectBySessionId(@Param("sessionId") String sessionId);
    ...
    @Update("UPDATE ai_chat_history SET is_deleted = 1, update_time = NOW(), updater = #{updater} " +
            "WHERE id = #{id} AND user_id = #{updater} AND is_deleted = 0")
