...
public class ReviewController {
    ...
    private final elm_bk.service.BusinessService businessService;

    @GetMapping("/public/business/{businessId}")
    public HttpResult<List<elm_bk.vo.PublicReviewVO>> publicReviews(@PathVariable Long businessId) {
        elm_bk.vo.PublicBusinessVO.from(businessService.getBusinessById(businessId));
        return HttpResult.success(reviewService.listByBusiness(businessId).stream()
                ...
                .map(elm_bk.vo.PublicReviewVO::from).toList());
    }
